...
public class RouteManagerDialog extends JDialog implements ActionListener, ListSelectionListener, TableModelListener,
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RouteManagerDialog.class);
    ...
    private void properties() {
        ...
        if (i >= 0) {
            RoutePropertiesDialog routePropertiesDialog = new RoutePropertiesDialog(this, routeManager, i);
    ...
    private void metocProperties() {
        int i = routeTable.getSelectedRow();
        if (i >= 0) {
            RouteMetocDialog routeMetocDialog = new RouteMetocDialog(this, routeManager, i);
    ...
    private void exportToFile(int routeId) {
        ...
        if(!RouteLoader.saveSimple(route, file)) {
            JOptionPane.showMessageDialog(EPDShip.getMainFrame(), "Route save error", "Route not saved", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void importFromFile() {
        ...
        JFileChooser fc = new JFileChooser(EPDShip.getHomePath().resolve(".aisview").toString() + "/routes");
//        private static final String aisViewFile = EeINS.getHomePath().resolve(".aisview").toString();
        fc.setFileSelectionMode(JFileChooser.FILES_ONLY);
