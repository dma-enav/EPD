...
public class SafeHavenPanel extends DockablePanel {
     ...
     */
    public enum State {
        INACTIVE(false, SafeHavenUtils.SF_COLOR_GRAY), OK(true, SafeHavenUtils.SF_COLOR_GREEN), WARN(true,
                SafeHavenUtils.SF_COLOR_YELLOW), ALERT(true, SafeHavenUtils.SF_COLOR_RED);
    ...
    public SafeHavenPanel() {
        ...
        // Title
        int gridy = 0;
        add(adjust(new JLabel("Safe Haven"), SwingConstants.CENTER, 14, Font.BOLD), new GridBagConstraints(0, gridy++, 1, 1, 1.0,
                0.0, CENTER, HORIZONTAL, insets0, 0, 0));

        // Safe haven view
        JPanel safeHavenViewPanel = new JPanel();
        safeHavenViewPanel.add(safeHavenView);
        add(safeHavenViewPanel, new GridBagConstraints(0, gridy++, 1, 1, 1.0, 0.0, CENTER, HORIZONTAL, insets5, 0, 0));

        // Target speed
        add(adjust(new JLabel("Target Speed"), SwingConstants.CENTER, 10, Font.PLAIN), new GridBagConstraints(0, gridy++, 1, 1,
                1.0, 0.0, CENTER, HORIZONTAL, insetsT5, 0, 0));
        add(adjust(lblTargetSpeed, SwingConstants.CENTER, 12, Font.BOLD), new GridBagConstraints(0, gridy++, 1, 1, 1.0, 0.0,
                CENTER, HORIZONTAL, insetsB5, 0, 0));

        // Present speed
        add(adjust(new JLabel("Present Speed"), SwingConstants.CENTER, 10, Font.PLAIN), new GridBagConstraints(0, gridy++, 1, 1,
                1.0, 0.0, CENTER, HORIZONTAL, insetsT5, 0, 0));
        add(adjust(lblPresentSpeed, SwingConstants.CENTER), new GridBagConstraints(0, gridy++, 1, 1, 1.0, 0.0, CENTER, HORIZONTAL,
                insetsB5, 0, 0));
    ...
    /**
     * Updates the panel with the speed, target speed and safe haven state. This method is run in the Swing event thread.
     */
    private synchronized void doUpdatePanel() {
        ...
        if (positionData == null || positionData.getPos() == null) {
            ...
            lblPresentSpeed.setText(String.format("(%s)", Formatter.formatCurrentSpeed((double) positionData.getSog(), 1)));
        }
        ...
        if (activeRoute == null || !activeRoute.isVisible() || Math.abs(activeRoute.getSafeHavenSpeed()) < 0.0000001) {
            ...
            if (activeRoute.getActiveWp().getOutLeg() != null) {
                sfWidth = activeRoute.getActiveWp().getOutLeg().getXtdPortMeters()
                        + activeRoute.getActiveWp().getOutLeg().getXtdStarboardMeters();
                sfLen = activeRoute.getActiveWp().getOutLeg().getSFLen();
            } else {
                sfWidth = activeRoute.getWaypoints().get(activeRoute.getWaypoints().size() - 2).getOutLeg().getXtdPortMeters()
                        + activeRoute.getWaypoints().get(activeRoute.getWaypoints().size() - 2).getOutLeg().getXtdStarboardMeters();
                sfLen = activeRoute.getWaypoints().get(activeRoute.getWaypoints().size() - 2).getOutLeg().getSFLen();
            }

            lblTargetSpeed.setText(String.format("%s", Formatter.formatCurrentSpeed(activeRoute.getSafeHavenSpeed(), 1)));
    ...
    protected Point2D computeShipPositionRelativeToSafeHaven() {
        ...
        pos.setLocation(distanceToShip * Math.cos(Math.toRadians(angleToShip)),
    ...
    class SafeHavenView extends JComponent {
         ...
         *            the label
         * @param direction
         *            the direction of the label text
         */
        ...
        private String formatTime(double dist) {
            Time t = new Dist(DistType.METERS, dist).withSpeed(new Speed(SpeedType.KNOTS, activeRoute.getSafeHavenSpeed())).in(
                    TimeType.MINUTES);
        ...
        public void paintComponent(Graphics g) {
            ...
            BasicStroke lineStroke = new BasicStroke(1.0f / (float) sfScale, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f,
                    new float[] { 4f / (float) sfScale, 2f / (float) sfScale }, 0.0f);
