...
public class VoyageLayer extends GeneralLayer implements
    ...
    public void findAndInit(Object obj) {
        ...
        if (obj instanceof JMapFrame) {
            mapContainer.getGlassPanel().add(voyageInfoPanel);
        }
    ...
    public boolean mouseMoved(MouseEvent e) {
        ...
        if (newClosest != closest && this.isVisible()) {
            Point containerPoint = SwingUtilities.convertPoint(chartPanel,
                    e.getPoint(), mapContainer.asComponent());
        ...
        if (monaLisaHandler != null && !windowHandling) {
            ...
            if (unhandledTransactions.size() > 0) {
                ...
                for (int j = 0; j < unhandledTransactions.size(); j++) {
                    ...
                    mapContainer.getGlassPanel().remove(shipIndicatorPanel);
                    mapContainer.getGlassPanel().add(shipIndicatorPanel);
                ...
                for (ShipIndicatorPanel value : shipIndicatorPanels.values()) {
                    mapContainer.getGlassPanel().remove(value);
                }
