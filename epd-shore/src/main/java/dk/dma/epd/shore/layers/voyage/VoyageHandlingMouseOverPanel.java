...
public class VoyageHandlingMouseOverPanel extends InfoPanel {
    ...
    public void showType(int type) {
        ...
        if (type == 2) {
            showText("Route suggested by Ship");
        }
    ...
    public void showVoyageInfo(Voyage voyage, String shipname) {
        ...
        try {
            ...
            // str.append(Formatter.latToPrintable(wp.getLatitude()) + " - " +
            // Formatter.lonToPrintable(wp.getLongitude()) + "<br/>");
            // str.append("<table border='0' cellpadding='2'>");
            // str.append("<tr><td>RNG:</td><td>" +
            // Formatter.formatDistNM(routeData.getRange(wpCircle.getIndex())) +
            // "</td></tr>");
            str.append("<tr><td>Between:</td><td>"
                    + Formatter.formatShortDateTime(route.getEtas().get(0))
                    + "</td></tr>");
            str.append("<tr><td>And:</td><td>"
                    + Formatter.formatShortDateTime(route.getEtas().get(
                            route.getEtas().size() - 1)) + "</td></tr>");
            str.append("</table>");
            str.append("</html>");
    ...
    public void showLegInfo(IntendedRouteLegGraphic legGraphic,
        ...
        str.append("<table border='0' cellpadding='2'>");
        str.append("<tr><td>DST:</td><td>" + Formatter.formatDistNM(range)
                + "  HDG " + Formatter.formatDegrees(hdg, 0) + "</td></tr>");
        str.append("<tr><td>START:</td><td>"
                + Formatter.formatShortDateTime(startEta) + "</td></tr>");
        str.append("<tr><td>ETA here:</td><td>"
                + Formatter.formatShortDateTime(midEta) + "</td></tr>");
        str.append("<tr><td>END:</td><td>"
                + Formatter.formatShortDateTime(endEta) + "");
        str.append("<tr><td>AVG SPD:</td><td>"
                + Formatter.formatSpeed(routeData.getSpeed(legIndex))
                + "</td></tr>");
        str.append("</table>");
        str.append("</html>");
