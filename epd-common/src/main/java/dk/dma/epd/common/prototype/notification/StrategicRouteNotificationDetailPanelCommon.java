...
public abstract class StrategicRouteNotificationDetailPanelCommon<T extends StrategicRouteNotificationCommon> extends NotificationDetailPanel<T> {
    ...
    public void setNotification(T notification) {
        ...
        CloudMessageStatus cloudStatus = (routeData.getLatestRouteMessage() != null) 
                ...
                : null;
        statusTxt.setText(getStatusType(routeData.getLatestRouteMessage(), cloudStatus, true));
    ...
    private void addStrategicRouteMessages(T notification) {
        ...
        for (int i = 0; i < routeData.getRouteMessage().size(); i++) {
            ...
            StrategicRouteMessageData routeMessage = routeData.getRouteMessage().get(routeIndex);
            StrategicRouteMessageData prevRouteMessage = (routeIndex == 0) ? null : routeData.getRouteMessage().get(routeIndex - 1);
            
            String routeChanges = 
                    (prevRouteMessage != null)
                    ? StrategicRouteNotificationCommon.findChanges(
                            new Route(prevRouteMessage.getMsg().getRoute()),
                            new Route(routeMessage.getMsg().getRoute()))
                    : null;
     ...
     */
    protected abstract String getMessageViewTitle(StrategicRouteMessageData routeMessage);

    
    /**
     * Converts the status into a textual description
     * @param routeData the route message
     ...
     */
    public static String getStatusType(StrategicRouteMessageData routeData, CloudMessageStatus cloudStatus, boolean useColor) {
        StringBuilder status = new StringBuilder();
        status.append("<html>");
        if (useColor) {
            status.append(String.format("<span style='color:%s'>%s</span>",
                    GraphicsUtil.toHtmlColor(routeData.getColor()),
                    routeData.getMsg().getStatus().toString()));
        } else {
            status.append(routeData.getMsg().getStatus().toString());
        }

        if (cloudStatus != null) {
            status.append("&nbsp;<small>(").append(cloudStatus.getTitle()).append(")</small>");
        }
...
class StrategicNotificationMessageView extends JPanel {
     ...
     * @param index the index of the message with 0 being the latest
     */
    public StrategicNotificationMessageView(
            String title,
            StrategicRouteMessageData routeMessage,
            String routeChanges,
            int index) {
        ...
        titlePanel.add(new JLabel(Formatter.formatLongDateTime(routeMessage.getSendDate())),
        ...
        add(new JLabel(StrategicRouteNotificationDetailPanelCommon.getStatusType(routeMessage, cloudStatus, false)),
                new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0, WEST, NONE, insets1, 0, 0));

        // Message label
        if (!StringUtils.isBlank(routeMessage.getMsg().getTextMessage())) {
            JLabel msgLabel = new JLabel("<html>" + Formatter.formatHtml(routeMessage.getMsg().getTextMessage()) + "</html>");
