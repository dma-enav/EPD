...
public enum AtoNTargetType {
    ...
    /* These are the actual AtoN types.
    Each type has an integer type code and a human-friendly name.
    */
     ...
     * @param atonTypeCode The AtoN type code.
     * @return The corresponding AtoNTargetType enum or null if atonTypeCode is an invalid type code.
     */
    public static AtoNTargetType getAtoNTargetTypeFromTypeCode(int atonTypeCode) {
        if(atonTypeCode >= 0 && atonTypeCode < AtoNTargetType.values().length)
            ...
            // Note that if the specification changes s.t. codes are no longer sequential, this must be updated.
